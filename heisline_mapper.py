...
k=0.099
#zero-points (instrumental base 25)
haZP=4.957
siiZP=5.077
...
while True:
    entered = raw_input("Ha zero-point (Base-25) (%.3f):\t" %haZP)
    try:
        haZP = float(entered)
        break
    except ValueError:
...
def toflux(imlist,kabs,amass,zerop):
    return np.multiply((3631.0/25.0),np.multiply((np.power(10.0,(kabs*amass/2.5))),(np.multiply((np.power(10.0,(zerop/2.5))),imlist))))
...
preratioerr=np.sqrt((np.square(np.divide(siierr,hasci)))+(np.square(np.divide((np.multiply(siisci,haerr)),(np.square(hasci))))))
...
ratioerr=np.sqrt((np.square(np.divide(siierr,hasci)))+(np.square(np.divide((np.multiply(siisci,haerr)),(np.square(hasci))))))
